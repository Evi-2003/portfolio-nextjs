...
const NavList = ({
  ...
  const { isNavOpen, toggleIsOpen } = useContext(NavContext);

  return (
    <>
      <ul className="flex w-full items-center justify-between md:w-auto">
        <li onClick={() => isNavOpen && toggleIsOpen()}>
          <Link href="/" className="w-full self-start text-2xl font-bold dark:text-white">
            &lt;evi-wammes/&gt;
          </Link>
        </li>
      ...
      <ul
        className={
          isNavOpen
            ? 'flex flex-col gap-2 text-center text-xl lg:flex lg:flex-row dark:text-white'
      ...
      >
        {allPaginas.map(
          (
            ...
            <NavItem key={pagina.id} label={pagina.label} slug={`/${pagina.slug}`} idx={idx} />
          ),
        )}
      </ul>
      <ul
...
<li onClick={() => toggleIsOpen()}>
          <Link href="https://github.com/Evi-2003" target="_blank" aria-label="Github van Evi Wammes">
            <button
              className="text-1xl mt-5 w-auto rounded-xl px-3 py-2 font-bold shadow-lg hover:bg-red-500 sm:mt-0
                sm:w-auto"
              aria-label="Github van Evi Wammes"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 dark:fill-white" viewBox="0 0 24 24">
        ...
        <li onClick={() => toggleIsOpen()}>
          <Link href="https://www.linkedin.com/in/eviwammes/" target="_blank" aria-label="LinkedIn van Evi Wammes">
            <button
              className="text-1xl mt-5 w-auto rounded-xl px-3 py-2 font-bold shadow-lg hover:bg-blue-400 sm:mt-0
                sm:w-auto"
              aria-label="LinkedIn van Evi Wammes"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 dark:fill-white" viewBox="0 0 24 24">
